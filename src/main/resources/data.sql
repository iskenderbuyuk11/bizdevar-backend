...
INSERT IGNORE INTO settings (setting_key, setting_value, group_name) VALUES
  ...
  ('language',        'az',       'general'),
  ('marketplace_name','BizdeVar', 'general');

-- Yegane resmi magaza (vendor id = 1) — butun mehsullar buna aiddir
INSERT IGNORE INTO vendors (id, user_id, name, category, verification_status, status, store_type, revenue, rating, auto_named)
VALUES (1, NULL, 'BizdeVar Resmi', 'Marketplace', 'verified', 'active', 'online', 184500, 4.8, 0);
...
-- Movcud bazani da yegane magazaya cevir (her acilisda idempotent islenir)
UPDATE products SET vendor_id = 1;
DELETE FROM vendors WHERE id <> 1;
UPDATE vendors SET name = 'BizdeVar Resmi', verification_status = 'verified', status = 'active' WHERE id = 1;
